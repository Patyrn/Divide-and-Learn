...
from qptl.qpthlocal.qp import QPFunction
from qptl.qpthlocal.qp import QPSolvers
from qptl.qpthlocal.qp import make_gurobi_model
...
class qptl_ICON:
    ...
    def fit(self, X, y, X_validation=None, y_validation=None, X_test=None, y_test=None):
        def make_model_matrix(nbMachines, nbTasks, nbResources, MC, U, D, E, L, P, idle, up, down, q, **h):
            ...
            # print(A.shape, G.shape,torch.transpose(F, 0, 1).shape)
        ...
        if validation:
            start_validation = time.time()
            solver_validation = Gurobi_ICON(relax=validation_relax, reset=True, presolve=False, **param)
        ...
        if test:
            start_test = time.time()
            solver_test = Gurobi_ICON(**param, reset=True, presolve=False)
            ...
            for i in range(n_knapsacks):
                ...
                if i % 50 == 0:
                    if self.verbose:
                        ...
                        if validation:
                            start_validation = time.time()
                            validation_result = self.test(X_validation, y_validation, sol_validation,
    ...
    def solution_func(self, y, solver, **params):
        ...
        for kn_nr in range(n_knap):
            ...
            # print('v',V)
            # print('solving model')
            tmp_solution = solver.solve_model(V)
            # print('solution',tmp_solution)
    ...
    def test(self, X, y, sol, solver):
        ...
        for i in range(n_knap):
            ...
            ### to be delted
            # print('regret',regret)
            # print('pred sol',sol_pred[i][0])
            # print('true sol',sol[i])
            ###########
            ...
            solver = QPFunction(verbose=False, solver=QPSolvers.GUROBI, model_params=model_params_quad)
            x = solver(Q.expand(n_train, *Q.shape),
                       c.squeeze(), G.expand(n_train, *G.shape), h.expand(n_train, *h.shape), torch.Tensor(),
                       torch.Tensor())
