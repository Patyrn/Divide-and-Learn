import sys
from IntOpt.Interior.qpthlocal.qp import QPFunction
from IntOpt.Interior.qpthlocal.qp import QPSolvers
from IntOpt.Interior.qpthlocal.qp import make_gurobi_model
from IntOpt.EnergyCost.ICON import *
from IntOpt.sgd_learner import *
from sklearn.metrics import mean_squared_error as mse
from collections import defaultdict
# from ip_model_whole import IPOfunc
from IntOpt.Interior.ip_model_whole import IPOfunc
...
from dnl.KnapsackSolver import knapsack_solver
...
def make_matrix_qp(nr_machines,nr_tasks,nr_resources,MC,U,D,E,L,P,idle,up,down,q,**h):
    ...
    Machines = range(nr_machines)
    Tasks = range(nr_tasks)
    Resources = range(nr_resources)
    N = 1440//q

    ### G and h
    G1 = torch.zeros((nr_machines*N ,nr_tasks*nr_machines*N))
    h1 = torch.zeros(nr_machines*N)
    F = torch.zeros((N,nr_tasks*nr_machines*N))
    for m in Machines:
        for t in range(N):
            h1[m*N+t] = MC[m][0]
            for f in Tasks:
                c_index = (f*nr_machines+m)*N
                G1[t + m*N, (c_index+max(0,t-D[f]+1)):(c_index+(t+1))] =1
                F [t,(c_index+max(0,t-D[f]+1)):(c_index+(t+1))  ] = P[f]
    G2 = torch.eye((nr_tasks*nr_machines*N))
    G3 = -1*torch.eye((nr_tasks*nr_machines*N))
    h2 = torch.ones(nr_tasks*nr_machines*N)
    h3 = torch.zeros(nr_tasks*nr_machines*N)

    G = torch.cat((G1,G2,G3)) 
    h = torch.cat((h1,h2,h3))

    ### A and b
    A1 = torch.zeros((nr_tasks, nr_tasks*nr_machines*N))
    A2 = torch.zeros((nr_tasks, nr_tasks*nr_machines*N))
    A3 = torch.zeros((nr_tasks, nr_tasks*nr_machines*N))

    for f in Tasks:
        A1 [f,(f*N*nr_machines):((f+1)*N*nr_machines) ] = 1
        A2 [f,(f*N*nr_machines):(f*N*nr_machines + E[f]) ] = 1
        A3 [f,(f*N*nr_machines+L[f]-D[f]+1):((f+1)*N*nr_machines) ] = 1
    b = torch.cat((torch.ones(nr_tasks),torch.zeros(2*nr_tasks)))
    A = torch.cat((A1,A2,A3))    
    return A,b,G,h,torch.transpose(F, 0, 1)

def make_matrix_intopt(nr_machines,nr_tasks,nr_resources,MC,U,D,E,L,P,idle,up,down,q,**h):
    # nbMachines: number of machine
    # nbTasks: number of task
    # nb resources: number of resources
    # MC[m][r] resource capacity of machine m for resource r
    ...
    ### G and h
    G = torch.zeros((nr_machines*N ,nr_tasks*nr_machines*N))
    h = torch.zeros(nr_machines*N)
    F = torch.zeros((N,nr_tasks*nr_machines*N))
    for m in Machines:
        for t in range(N):
            h[m*N+t] = MC[m][0]
...
def knap_solution(param, y, weights, relax, n_items):
    capacity = param.get('capacity')
    # knapsack_solver(y, weights, capacity)
    ...
    for kn_nr in range(n_knap):
        ...
        _, sol = knapsack_solver(V.tolist(), weights.tolist(), capacity)
...
def knap_to_onehot(sol, y):
    one_hot = np.zeros(len(y))
    for i in sol:
        one_hot[i] = 1
    return one_hot

def validation_module_knap(param, n_items, epoch=None, batch=None,
                      model_time=None, run_time=None,
                      y_target_validation=None, sol_target_validation=None, y_pred_validation=None, weights_validation=None,
                      y_target_test=None, sol_target_test=None, y_pred_test=None, weights_test =None,
                      validation_relax=False, **kwargs):
    def regret(y_target, sol_target, y_pred, weights, relax=False, **kwargs):
        ...
        for kn_nr in range(n_knap):
            ...
            _, sol_target = knapsack_solver(V_target.tolist(), weights.tolist(), capacity)
            sol_target = knap_to_onehot(sol_target,V)
            _, sol = knapsack_solver(V.tolist(), weights.tolist(), capacity)
            sol = knap_to_onehot(sol,V)
            true_list.append(sum(V_target * sol_target))
            # regret_list.append(sum(V_target*-(sol-sol_target[kn_nr])))
            regret_list.append(sum(V_target * -(sol - sol_target)))
            # print('sol', sol)
            # print('sol_target', sol_target[kn_nr])
            # print(regret_list)
        # print('mean true', np.mean(true_list))
        # print("regret", np.median(regret_list))
    ...
    if (y_pred_validation is not None) and (y_target_validation is not None) and (sol_target_validation is not None):
        ...
        validation_result = regret(y_target_validation, sol_target_validation,
                                   y_pred_validation, weights_validation, relax=validation_relax)
    ...
    if (y_pred_test is not None) and (y_target_test is not None) and (sol_target_test is not None):
        ...
        test_result = regret(y_target_test, sol_target_test, y_pred_test, weights_test,
                             relax=False)
...
def validation_module(param,n_items,epoch=None, batch=None, 
    ...
    def regret(y_target,sol_target,y_pred,relax= False,**kwargs):
        clf =  Gurobi_ICON(relax=relax,method=-1,reset=True,presolve=False,**param)
        clf.make_model()
        n_knap = len(y_pred)//n_items
        regret_list= []
        true_list = []
        for kn_nr in range(n_knap):
            ...
            V = y_pred[kn_start:kn_stop]
            V_target = y_target[kn_start:kn_stop]
            sol_target,_ = clf.solve_model(V_target)
            sol,_ = clf.solve_model(V)
            true_list.append(sum(V_target*sol_target))
            # regret_list.append(sum(V_target*-(sol-sol_target[kn_nr])))
            regret_list.append(sum(V_target * -(sol - sol_target)))
...
def return_dict_validation_knapsack(instance,model,predict,param,n_items,
    model_save, validation, test,
     model_name="",validation_relax = True,
                           X_validation=None, y_validation=None, weights_validation=None, X_test=None, y_test=None, weights_test=None):
    if validation:
        y_pred_validation = predict(X_validation, doScale=False)
        if not hasattr(instance, 'sol_validation'):
            # instance.sol_validation = ICON_solution(param=param, y=y_validation,
            #                                         relax=validation_relax, n_items=n_items)
            instance.sol_validation = knap_solution(param = param,y = y_validation, weights= weights_validation,
                    relax = validation_relax,n_items = n_items)
    else:
        instance.sol_validation = None
        y_pred_validation = None
    if test:
        ...
        if not hasattr(instance, 'sol_test'):
            # instance.sol_test = ICON_solution(param=param, y=y_test,
            #                                   relax=False, n_items=n_items)
            instance.sol_test = knap_solution(param = param,y = y_test, weights= weights_test,
                    relax = validation_relax,n_items = n_items)
    else:
        instance.sol_test = None
        y_pred_test = None
...
class intopt_energy:
    ...
    def fit_knapsack(self, X, y, weights, X_validation=None, y_validation=None, weights_validation = None, X_test=None, y_test=None, weights_test = None):
        self.model_time = 0.
        runtime = 0.
        capacity = self.param.get('capacity')
        validation_time = 0
        test_time = 0
        ...
        for e in range(epochs):
            ...
            for i in range(n_batches):
                ...
                for j in batch_list:
                    ...
                    y_tensor =torch.tensor(y[n_start:n_stop].flatten(), dtype=torch.float)

                    c_pred = self.model(X_tensor).squeeze()
                    ...
                    with time_limit(self.problem_timelimit):
                        ...
                        G = torch.tensor(weights.flatten(),dtype=torch.float).unsqueeze(0)
                        h = torch.tensor([X_tensor.shape[0] * capacity[0]], dtype=torch.float)

                        A = torch.Tensor()
                        b = torch.Tensor()
                        #
                        # x = IPOfunc(A, b, G, h, pc=True, max_iter=self.max_iter, bounds= [(0., 1.)],
                        ...
                        x = IPOfunc(G, h, A, b, bounds=[(0., 1.)], max_iter=self.max_iter, thr=self.thr,
                                    damping=self.damping,
                                    smoothing=self.smoothing)(c_pred)
                        # print('after IPOfunc')
                        loss = (x * y_tensor).mean()
                        c_pred.retain_grad()
            ...
            if self.store_validation:
                y_pred_validation, y_pred_test = return_dict_validation_knapsack(self, self.model, self.predict, self.param, self.n_items,
                                                                                 self.model_save, validation, test,
                                                                                 self.model_name, self.validation_relax,
                                                                                    X_validation, y_validation, weights_validation, X_test, y_test, weights_test)



                dict_validation = validation_module_knap(param=param, n_items=self.n_items,
                                                    run_time=runtime, epoch=e, batch=i,
                                                    model_time=self.model_time,
                                                    y_target_validation=y_validation,
                                                    sol_target_validation=self.sol_validation,
                                                    y_pred_validation=y_pred_validation, weights_validation=weights_validation,
                                                    y_target_test=y_test, sol_target_test=self.sol_test,
                                                    y_pred_test=y_pred_test, weights_test =weights_test,  validation_relax=self.validation_relax)
