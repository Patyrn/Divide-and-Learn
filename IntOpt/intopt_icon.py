...
import os
import sys
from IntOpt.Interior.intopt_energy_mlp import intopt_energy, qptl_energy, SPO_energy, twostage_energy
...
def intopt_icon_run(train_set, test_set, n_iter=10, instance_number=1, opt_params=None, file_name_suffix='empty',
                dest_folder="Tests/icon/Easy/intopt/", time_limit=12000, epoch_limit=1):
    ...
    test_rslt=clf.fit(X=X_1gtrain, y=y_train,X_validation=X_1gvalidation,y_validation=y_validation,X_test=X_1gtest,y_test=y_test)
    rslt = test_rslt
    ...
    test_rslt = clf.fit(X=X_1gtrain, y=y_train, X_validation=X_1gvalidation, y_validation=y_validation, X_test=X_1gtest,
                        y_test=y_test)
    rslt = test_rslt
