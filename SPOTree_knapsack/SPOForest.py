...
"""
import numpy as np
from SPOTree_knapsack.mtp import MTP
from SPOTree_knapsack.decision_problem_solver import*
from SPOTree_knapsack.SPO_tree_greedy import SPOTree
...
class SPOForest(object):
  ...
  '''
  def est_decision(self, Xnew, method="mean", item_weights=None,Y=None):
    if method == "mean":
      forest_costs = self.est_cost(Xnew)
      print('Xnew',Xnew.shape)
      print("Cost", forest_costs.shape)
      import matplotlib.pyplot as plt
      sample, feat = Xnew.shape

      for i in range(sample):
        plt.subplot(3,1,1)
        tmp_Ynew = forest_costs[i,:]
        plt.plot(tmp_Ynew)
        # plt.subplot(3, 1, 2)
        # tmp_Xnew = Xnew[i, :]
        # plt.plot(tmp_Xnew)
