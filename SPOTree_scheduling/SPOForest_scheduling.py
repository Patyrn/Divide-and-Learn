...
from SPOTree_scheduling.SPOForest import SPOForest

from SPOTree_scheduling.decision_problem_solver import find_opt_decision
...
def forest_traintest(train_x, train_cost, test_x, test_cost, n_estimators, max_depth,
                     min_samples_leaf, max_features, run_in_parallel, num_workers, algtype, quant_discret, opt_params):
    if algtype == "MSE":
    ...
    regr.fit(train_x, train_cost, verbose_forest=True, verbose=False, feats_continuous=True,
             seed=forest_seed, opt_params=opt_params)
...
def forest_tuneparams(train_x, train_cost, valid_x, valid_cost, n_estimators_set,
                      max_depth_set, min_samples_leaf_set, max_features_set, run_in_parallel, num_workers, algtype,
                      quant_discret, opt_params ):
            ...
            for min_samples_leaf in min_samples_leaf_set:
                for max_features in max_features_set:
                    ...
                    regr, err_mean = forest_traintest(train_x, train_cost, valid_x, valid_cost,
                                                      ...
                                                      num_workers, algtype, quant_discret, opt_params)
...
# '''
def SPOTree_scheduling(max_depth_set_str="1000", min_samples_leaf_set_str="10000",
                       n_estimators_set_str="50", max_features_set_str="2-3-4-5", algtype="SPO", core_number=4,
                       decision_problem_seed=-1, train_size=-1, quant_discret=0.05, load=None, kfold=0, file_folder = "Tests/spotree/"):
    if load == None:
        opt_params =  get_icon_instance_params(30)
    else:
    ...
    ########################################
    # output filename
    fname_out_mean = algtype + "Forest_news_costs_depthSet" + max_depth_set_str + "_minObsSet" + min_samples_leaf_set_str + "_nEstSet" + n_estimators_set_str + "_mFeatSet" + max_features_set_str + "_aMethod" + "mean" + "_probSeed" + str(
    ...
    dataset = get_energy_data('../data/energy_data.txt', generate_weight=True, unit_weight=True, kfold=kfold,
                              is_spo_tree=True)
    ...
    regr_mean, best_err_mean, best_n_estimators_mean, best_max_depth_mean, best_min_samples_leaf_mean, best_max_features_mean, best_run_time = forest_tuneparams(train_x, train_cost, valid_x, valid_cost, n_estimators_set,
                                                 ...
                                                 algtype, quant_discret, opt_params)
    ...
    print(test_x)
    # FIND TEST SET COST
    pred_decision_mean = regr_mean.est_decision(test_x, opt_params=opt_params, method="mean")
    # test_x = test_x * 10 + 23
    # pred_decision_mean2 = regr_mean.est_decision(test_x, method="mean",opt_params=opt_params)
    # pred_decision_mode = regr_mode.est_decision(test_x, method="mode")
    true__preds = find_opt_decision(values_arr=test_Y, opt_params=opt_params)['weights']
    ...
    print(true_costs_mean)
    print(costs_mean)
    print(true_costs_mean-costs_mean)
    ...
    fname_out_mean = "spotree_"+ "icon_l" + str(load) + "k" + str(kfold) + ".csv";

    fname = file_folder + fname_out_mean
    fname = os.path.join(dir_path, fname)
    regret = np.median(true_costs_mean - costs_mean)

    save_output_spotree_knap(fname, best_err_mean, best_n_estimators_mean, best_max_depth_mean,
                             best_min_samples_leaf_mean, best_max_features_mean, best_run_time, regret)
