...
from SPO.sgd_learner import *
from torch import nn, optim
from collections import defaultdict


class SGD_SPO_generic:
    def __init__(self, param, solver, reset, presolve, relax=False, validation_relax=False, test_relax=False,
                 optimal_value=knapsack_value, getY=get_profits, getTorchData=get_data,
    ...
    def validation_score(self, criterion, trch_X_validation=None, trch_y_validation=None, trch_X_test=None,
                         trch_y_test=None,
        ...
        if trch_X_validation is not None:
            ...
            for kn_nr in range(n_knap_validation):
                ...
                if t is not None:
                    self.time += t
                regret_full_validation[kn_nr] = max_min_ind * (
                            optimal_value(V_true, sol_true, **param) - optimal_value(V_true, sol_pred, **param))
    ...
    def test_score(self, criterion, trch_X_test=None, trch_y_test=None, knaps_sol_test=None, pool=None):
        ...
        if trch_X_test is not None:
            ...
            if pool is None:
                for kn_nr in range(n_knap_test):
                    ...
                    sol_true = knaps_sol_test[kn_nr][0]
                    sol_pred, _ = clf_test.solve_model(V_pred)
                    regret_full_test[kn_nr] = max_min_ind * (
                                optimal_value(V_true, sol_true, **param) - optimal_value(V_true, sol_pred, **param))
            else:

                V_trues = [self.getY(trch_y_test, kn_nr, n_items) for kn_nr in range(n_knap_test)]
                V_preds = [self.getY(V_preds_test, kn_nr, n_items) for kn_nr in range(n_knap_test)]

                map_func = partial(test_score_worker, max_min_ind=max_min_ind, param=param)

                iter = zip(V_trues, V_preds)
                regret_full_test = pool.starmap(map_func, iter)
                print('regret full', regret_full_test)
                regret_full_test, optimal_objective_values = zip(*regret_full_test)
                print('regret full', regret_full_test)
                # print('true obj',np.sum(np.concatenate(optimal_objective_values)))
            print('test regret:', np.median(regret_full_test))
            info['test_regret_full'] = np.median(regret_full_test)
        return info
    ...
    def fit(self, x_train, y_train, x_validation=None, y_validation=None, x_test=None, y_test=None):
        ...
        if validation:
            ...
            knaps_V_true_validation = [self.getY(trch_y_validation, kn_nr, n_items) for kn_nr in
                                       range(n_knapsacks_validation)]
        ...
        for epoch in range(num_epochs):
            ...
            for kn_nr in knapsack_nrs:
                ...
                if self.timelimit is not None and (time.time() - start - test_time) >= self.timelimit:
                ...
                if self.verbose:
                    ...
                    if cnt % 50 == 0:
                        ...
                        if validation:
                            validation_start = time.time()
                            dict_val = self.validation_score(criterion, trch_X_validation=trch_X_validation,
                                                             trch_y_validation=trch_y_validation,
                                                             knaps_sol_validation=knaps_sol_validation,
                            ...
                            print(
                                "Epoch %d::subepoch %d Total time %d, validation time %d & test time %d validation regret:%.2f" % (
                                epoch,
                                subepoch, time.time() - start, validation_time, test_time,
                                dict_validation['validation_regret_full']))
                        if test:
                            test_start = time.time()
                            print('Min val regret', min_val_regret, 'c')
                            if dict_validation['validation_regret_full'] < min_val_regret:
                                #change this to revert
                                min_val_regret = dict_validation['validation_regret_full']
                                dict_test = self.test_score(criterion, trch_X_test, trch_y_test, knaps_sol_test)
                            else:
                                # print('hey', dict_validation['validation_regret_full'])
                                dict_test = test_result[-1]
                            # dict_test = self.test_score(criterion, trch_X_test, trch_y_test, knaps_sol_test)
                        ...
                        dict_validation["Runtime"] = time.time() - start - test_time
                        dict_validation["time"] = time.time() - start
                        test_result.append(dict_validation)
                        logging.info(
                            "Epoch %d::subepoch %d Total time %d, validation time %d & test time %d" % (epoch, subepoch,
                                                                                                        time.time() - start,
                                                                                                        validation_time,
                                                                                                        test_time))
                        ### delete
                ...
                if self.model_save:
                    if cnt % 10 == 0:
                        ...
                        torch.save(self.model.state_dict(),
                                   str(self.model_name + "_Epoch" + str(epoch) + "_" + str(subepoch) + ".pth"))
        if self.verbose:
            dd = defaultdict(list)
...
def test_score_worker(V_trues, V_preds, max_min_ind, param):
    regret_full = np.zeros(len(V_preds))
    clf_test = 1
    for i, (V_pred, V_true) in enumerate(zip(V_preds, V_trues)):
        sol_pred, _ = ICON_scheduling(V_pred,**param)
        sol_true, _ = ICON_scheduling(V_true,**param)

        regret_full[i] = max_min_ind * (
                optimal_value(V_true, sol_true, **param) - optimal_value(V_true, sol_pred, **param))
    regret_full = 1
    print('regret',regret_full)
    return regret_full
