...
from dnl.KnapsackSolver import knapsack_solver, calculate_profit_from_items
from SPO.KnapsackSolving import *
...
def get_kn_indicators(V_pred, c, weights=None,use_dp= True,relaxation=False,warmstart=None):
    ...
    if use_dp:
        start_time = tm.time()
        if relaxation:
            solutions = solveKnapsackProblemRelaxation(V_pred,weights,c,warmstart=warmstart)
        else:
            solutions = solveKnapsackProblem(V_pred,weights,c,warmstart=warmstart)
        # runtime, assignments_pred_or = knapsack_solver(V_pred, weights=[weights[0].tolist()], capacities=c)
        # assignments = [0] * 48
        # for i in assignments_pred_or:
        #     assignments[i] = 1
        # runtime = tm.time()-start_time
        return np.asarray(solutions['assignments']),solutions['runtime']
        # return np.asarray(assignments), runtime
...
def test_fwd(model, criterion, trch_X, trch_y, n_items, capacity, knaps_sol,weights=None,relaxation=False):
    ...
    true_proft_gur = np.zeros(n_knap)
    regret_gur = np.zeros(n_knap)
    pred_proft_gur = np.zeros(n_knap)

    true_proft_or = np.zeros(n_knap)
    regret_or = np.zeros(n_knap)
    pred_proft_or = np.zeros(n_knap)
    ...
    for kn_nr in range(n_knap):
        ...
        #use this only not to break functions
        assignments_pred, t = get_kn_indicators(V_pred, c=capacity, weights=weights, relaxation=relaxation)
        weight_array = weights[0]
        # if relaxation:
        #     # print(np.array(weight_array * assignments_pred).astype(float))
        #     # print(assignments_pred)
        #     if np.sum(np.array(weight_array * assignments_pred).astype(float)) > capacity:
        #         print('total weights',np.sum(np.array(weight_array * assignments_pred).astype(float)))
        #using it to not to break functions
        assignments_true = knaps_sol[kn_nr][0]
        # if not relaxation:
        #     print('assignments',np.sum(assignments_true - assignments_pred), 'relaxation', relaxation)
        #     if (np.sum(assignments_true - assignments_pred)):
        #         print('true',assignments_true)
        #         print('true',assignments_pred)
        # regret_full[kn_nr] = np.sum((V_true * assignments_true) - (V_true * assignments_pred))
        # if not relaxation and (regret_full[kn_nr]) < 0:
        #     print('WARNING REGRET NEGATIVE')
        # if relaxation:
        #     regret_full[kn_nr] = np.sum(V_true * (assignments_true - assignments_pred))
        # else:

        __,assignments_true_or = knapsack_solver(V_true,weights=[weights[0].tolist()],capacities=capacity)
        start_time = tm.time()
        __, assignments_pred_or = knapsack_solver(V_pred, weights=[weights[0].tolist()], capacities=capacity)
        true_proft_or[kn_nr] = calculate_profit_from_items(assignments_true_or, V_true)
        pred_proft_or[kn_nr] = calculate_profit_from_items(assignments_pred_or, V_true)
        regret_full[kn_nr] = true_proft_or[kn_nr] - pred_proft_or[kn_nr]

        # if not relaxation and (regret_full[kn_nr]) < 0:
        #     print('WARNING REGRET  OR', regret_full[kn_nr])

        if not relaxation :
            # cf = confusion_matrix(assignments_true_or, assignments_pred_or,labels=[0,1])
            #not using anymore
            cf = confusion_matrix([0,1,0], [1,0,1],labels=[0,1])
            cf_list.append(cf)
    ...
    # print('GUR test true obj', np.sum(true_proft_gur))
    # print('GUR test pred obj', np.sum(pred_proft_gur))
    # print('GUR regret', np.median(true_proft_gur-pred_proft_gur))

    # print('OR test true obj', np.sum(true_proft_or))
    # print('OR test pred obj', np.sum(pred_proft_or))
    # print('OR regret', np.median(true_proft_or-pred_proft_or))

    # print('regret gurobi', np.median(regret_full))
    info['nonzero_regrsm'] = sum(regret_smooth != 0)
    info['nonzero_regrfl'] = sum(regret_full != 0)
    #info['regret_smooth'] = np.average(regret_smooth)
    # if (np.min(regret_full) < 0 and not relaxation):
    #     print(regret_full)
    #     print(np.median(regret_full))
